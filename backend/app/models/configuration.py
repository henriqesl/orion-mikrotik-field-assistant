...
class LinkConfiguration(BaseModel):
    ...
    def validate_network(self):
        ...
        if self.wifi_interface in self.bridge_interfaces:
            raise ValueError("A interface Wi-Fi já é incluída automaticamente na bridge.")
        if self.device_kind == "radio" and not self.manage_topology:
            raise ValueError("A configuração de enlace exige bridge e IP de gerenciamento.")
        if self.device_kind == "radio" and not self.bridge_interfaces:
            raise ValueError("Selecione pelo menos uma interface cabeada para o enlace.")
...
class BasicNetworkConfiguration(BaseModel):
    ...
    dns_servers: list[IPv4Address] = Field(max_length=3)
...
class BasicNetworkCurrentState(BaseModel):
    ...
    configure_lan: bool
    lan_bridge: str | None = None
    lan_address: str | None = None
    lan_ports: list[str] = Field(default_factory=list)
    dns_servers: list[str] = Field(default_factory=list)
    enable_nat: bool
    enable_lan_dhcp: bool
    dhcp_pool_start: str | None = None
    dhcp_pool_end: str | None = None
    enable_ssh: bool
    enable_winbox: bool
    enable_webfig_https: bool
    enable_telnet: bool
    enable_ftp: bool
    enable_webfig_http: bool
